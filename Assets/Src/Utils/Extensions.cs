...
namespace Suburb.Utils
{
    ...
    {
        public static bool IsClose(this Vector2 source, Vector2 destination, float closeDistance)
        {
            Debug.Log($"a: {source}, b: {destination}");
            Debug.Log($"max :{ Mathf.Max(Mathf.Abs(source.x - destination.x), Mathf.Abs(source.y - destination.y))}");

            return Mathf.Abs(source.x - destination.x) < closeDistance 
                && Mathf.Abs(source.y - destination.y) < closeDistance;
        }

        public static bool IsClose(this Vector3 source, Vector3 destination, float closeDistance)
        {
            return Mathf.Abs(source.x - destination.x) < closeDistance
                && Mathf.Abs(source.y - destination.y) < closeDistance
